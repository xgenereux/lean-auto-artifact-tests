...
### Structure
- **Index**: Multi-index with `(tactic, declaration)`
- **Rows**: 1,755,090 records (175,509 declarations × 10 tactics)
...
**Tactics** (10 variants, in order):
1. `testUnknownConstant` - Test for unknown constants (141,613 successful)
2. `useRfl` - Reflexivity tactic
3. `useSimpAll` - Simplification with all hypotheses
4. `useSimpAllWithPremises` - Simplification with premises
5. `useAesop` - Basic Aesop tactic
6. `useAesopWithPremises` - Aesop with premises
7. `useAesopPUnsafeNew` - New unsafe Aesop variant (45,861 successful)
8. `useAesopPUnsafeOld` - Old unsafe Aesop variant
9. `useSaturateNewDAss` - New saturation variant (13,120 successful)
10. `useSaturateOldDAs` - Old saturation variant
